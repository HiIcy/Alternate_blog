...
def get_user_posts(id):
	...
	if pagination.has_prev:
		prev = url_for('api.ger_user_posts', page=page - 1, _external=True)
	next = None
	if pagination.has_next:
		next = url_for('api.ger_user_posts', page=page - 1, _external=True)
	return jsonify({
...
def ger_user_followed_posts(id):
	user = User.query.get_or_404(id)
	page = request.args.get('page', 1, type=int)
